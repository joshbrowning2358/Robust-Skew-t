...
sdval=NULL
for (i in 1:p){
sdval=rbind(sdval,sbiv(u,v,i))
}

#compute depth value as weighted average
fdval=matrix(wbiv(sdval),p,n,byrow=T)
#singel level method
out.s=outdp(sdval,u,v,ff)
#functional approach
out.f=outdp(fdval,u,v,ff)
return(list(out.s=out.s,out.f=out.f))
}
...
#MVST; remove the whole outlying launches
mst=function(u,v,af=0.025,rm.out=F,out.dp){
np=dim(u)
p=np[1]
n=np[2]
uc=u
