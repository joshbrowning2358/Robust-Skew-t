...
mm = out.method
  tab=NULL
load( paste0("simulation_j_951_mm_",mm,"_mod_",mod,"_pout_",p.out,"_out_",out.method,"_k_",kVal,".RData") )
cat("Initializing simulations.  dim(tab):")
print(dim(tab))
  for(j in 952:nsim){
    ...
    if(j %% 50==1 )
      save(tab, conv, file=paste0("simulation_j_",j,"_mm_",mm,"_mod_",mod,"_pout_",p.out,"_out_",out.method,"_k_",kVal,".RData"))
