...
robustST = function(y, x=matrix(1,nrow=NROW(y)), robust=T, method=c("constrOptim", "nlminb"), w=rep(1,nrow(x)), k=10
            ,start=NULL){
  #Data quality checks
  if(!is(x,"matrix"))
...
robustST( y=matrix(rnorm(200), ncol=2) )
robustST( y=rnorm(100) )
